...
```{r}
gb <- fread("../input/GBvideos.csv",encoding = "UTF-8")
gb[,"Location":="GB"]
fr <- fread("../input/FRvideos.csv",encoding = "UTF-8")
fr[,"Location":="FR"]
ca <- fread("../input/CAvideos.csv",encoding = "UTF-8")
ca[,"Location":="CA"]
us <- fread("../input/USvideos.csv",encoding = "UTF-8")
us[,"Location":="US"]
de <- fread("../input/DEvideos.csv",encoding = "UTF-8")
de[,"Location":="DE"]
...
## Viewed videos
```{r}
mvideo <- videos[,.("Total_Views"=round(sum(views,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Total_Views)]
...
```{r}
mvideo <- videos[,.("Total_Likes"=round(sum(likes,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Total_Likes)]
...
```{r}
mvideo <- videos[,.("Total_Dislikes"=round(sum(dislikes,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Total_Dislikes)]
...
```{r}
mvideo <- videos[,.("Total_comments"=round(sum(comment_count,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Total_comments)]
...
## % Liked videos
```{r}
mvideo <- videos[,.("Percentage_Likes"=round(100*sum(likes,na.rm = T)/sum(views,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Percentage_Likes)]
...
## % Disliked videos 
```{r}
mvideo <- videos[,.("Percentage_Dislikes"=round(100*sum(dislikes,na.rm = T)/sum(views,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Percentage_Dislikes)]
...
## % Commented videos 
```{r}
mvideo <- videos[,.("Percentage_comments"=round(100*sum(comment_count,na.rm = T)/sum(views,na.rm = T),digits = 2)),by=.(title,thumbnail_link)][order(-Percentage_comments)]
...
* There are mainly bigrams relationed to music, also we can see the *Iphone X* , tv series like *stranger things* and games like *fifa*.
...
# Tags wordcloud
```{r include=FALSE}
corpus = Corpus(VectorSource(list(videos$tags)))
corpus = tm_map(corpus, removePunctuation)
corpus = tm_map(corpus, content_transformer(tolower))
corpus = tm_map(corpus, removeNumbers) 
corpus = tm_map(corpus, stripWhitespace)
...
# Views Vs Likes
```{r}
ggplot(videos[,.("views"=mean(views),"likes"=mean(likes)),by=title],aes(views,likes,colour=likes,size=likes))+geom_jitter()+geom_smooth()+guides(fill="none")+labs(caption="Donyoe",title="Views Vs Likes",subtitle="In days")+theme(legend.position = "none")+geom_text(data=subset(videos[,.("views"=mean(views),"likes"=mean(likes)),by=title], views > 1.5e+07),
            aes(views,likes,label=title),check_overlap=T)
```

# Likes Vs Comment
```{r}
ggplot(videos[,.("comment_count"=mean(comment_count),"likes"=mean(likes)),by=title],aes(comment_count,likes,colour=likes,size=likes))+geom_jitter()+geom_smooth()+guides(fill="none")+labs(caption="Donyoe",title="Views Vs Comment",subtitle="In days")+
  theme(legend.position = "none")+geom_text(data=subset(videos[,.("comment_count"=mean(comment_count),"likes"=mean(likes)),by=title], likes > 500000),
            aes(comment_count,likes,label=title),check_overlap=T)
```

# Sentiment Analysis Description field
```{r include=FALSE}
corpus = Corpus(VectorSource(list(videos$description)))
corpus = tm_map(corpus, removePunctuation)
corpus = tm_map(corpus, content_transformer(tolower))
corpus = tm_map(corpus, removeNumbers) 
corpus = tm_map(corpus, stripWhitespace)
...
* Cluster 1 : The second in reach the trending page with 4.3 days, and medium in views, likes and dislikes.
* Cluster 2 : The cluster with most views with a significative difference, very liked, disliked and commented as in absolute as in relative units.
* Cluster 3 : This is a quite weird cluster, they reach the trending page in 32 days in mean, they have very little views, likes, dislikes and comments, and is the last in percentage of likes, dislikes and comments.
...
```{r}
ggplot(videos[,.("Total_Views"=sum(views)),by=Location],aes(reorder(Location,-Total_Views),Total_Views,fill=Location))+geom_bar(stat="identity")+geom_label(aes(label=Total_Views))+guides(fill="none")+theme(axis.text.x = element_text(angle = 45,hjust = 1))+  labs(caption="Donyoe",title=" Total Views by Countries")+xlab(NULL)+ylab(NULL)
```
