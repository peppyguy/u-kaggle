...
The US is one of the few countries in which the right to bear arms is constitutionally protected. A consequence of this is that the level of gun voilence is very high compared to other countries. After analyzing the dataset, which was downloaded from gunviolence.org, my main findings were:
...
#Note: In cases in which I only needed one particular function from a package, I decided to not load the whole package but only load the function that I need using the package::function() notation.
```
...
#adding the missing Las Vegas incident manually. The dataset poster explains why he took this incident out in this discussion topic: [Las Vegas incident missing](https://www.kaggle.com/jameslko/gun-violence-data/discussion/55307).
...
In order be able to relate the numbers above to the population sizes, I had to look for a good source. I found what I needed on the website of the US Census Bureau. Directly reading the data into a dataframe did not work in the Kaggle kernel. Therefore, I uploaded it to a private dataset (the link to it is set in the beginning of the document. path2 <- "../input/erik-gun-extra/").
...
Below, I am creating a small "states" dateframe, that eventually holds the number of incidents relative to the population of each state (per 100,000 inhabitants).
...
```{r}
head(states@data) #please notice the different notation using `@` instead of the usual '$'
```

With this datastructure, a straightforward join (joining the Per100000 column) with additional data is not possible. However, I found out that an easy workaround (for the time being) is to extract data elements from the object, join this small dataframe with the info that I want to display on the map (the incidents per 100,000 inhabitants), and add this column in the 'states' object.
...
grid.arrange(usOverallCats, louisianaCats, delawareCats, alaskaCats, ncol=1)
```
