...
train %>% ggplot(aes(x=price_doc))+
  geom_histogram(bins=1000, fill="darkblue")+
...
### Zoomed
When zooming closer one can clearly see the two spikes on the left side of the distribution. These seem to be faked sale prices to avoid tax and are one
of the reasons why local CV and LB don't match well.
```{r warning = FALSE, message = FALSE, cache = FALSE}
train %>% ggplot(aes(x=price_doc))+
  geom_histogram(bins=200, fill="grey60")+
...
missing_values %>% 
  ggplot(aes(x=reorder(feature,missing_pct),y=missing_pct, group=train_test, fill=train_test)) +
...
full %>% filter(num_room <= 5) %>% ggplot(aes(x=num_room)) + stat_density(geom="line",position="identity", aes(group=train_test,color=train_test),size=1.2,na.rm=T)+facet_grid(product_type~.)+theme_bw()

full %>% filter(floor <= 35) %>% ggplot(aes(x=floor)) + stat_density(geom="line",position="identity", aes(group=train_test,color=train_test),size=1.2,na.rm=T)+facet_grid(product_type~.)+theme_bw()

full %>% filter(max_floor <= 35) %>% ggplot(aes(x=max_floor)) + stat_density(geom="line",position="identity", aes(group=train_test,color=train_test),size=1.2,na.rm=T)+facet_grid(product_type~.)+theme_bw()
