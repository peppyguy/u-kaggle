...
fastgreed <- vector()
leadingeig <- vector()
num_of_filtrations <- 100
for (k in 1:num_of_filtrations){
  edgemk <- edgem
  cutk <- quantile(as.vector(edgem),k/num_of_filtrations)
  g_fk <- filter(cutoff = cutk,
                edge_matrix = edgem,
                vertex_colors = V(g)$color,
                vertex_names = V(g)$name,
                vertex_size = V(g)$size)
  mc <- multilevel.community(g_fk)
  wc <- walktrap.community(g_fk)
  fg <- fastgreedy.community(g_fk)
  le <- leading.eigenvector.community(g_fk)
  multilevel[k] <- length(unique(mc$membership))
  walktrap[k] <- length(unique(wc$membership))
  fastgreed[k] <- length(unique(fg$membership))
  leadingeig[k] <- length(unique(le$membership))
}
par(mfrow=c(2,2))
plot(1:num_of_filtrations,
     multilevel,
     pch = 21,
     cex = 1.5,
     col = "darkgreen",
     xlab = "Threshold Value Index",
     ylab = "Number of Clusters",
     main = "Multilevel Community Number of Clusters")
plot(1:num_of_filtrations,
     walktrap,
     pch = 21,
     cex = 1.5,
     ...
     xlab = "Threshold Value Index",
     ylab = "Number of Clusters",
     main = "Walktrap Community Number of Clusters")
plot(1:num_of_filtrations,
     fastgreed,
     col = "red",
     pch = 21,
     ...
     xlab = "Threshold Value Index",
     ylab = "Number of Clusters",
     main = "Fastgreedy Community Number of Clusters")
plot(1:num_of_filtrations,
     leadingeig,
     pch = 21,
     cex = 1.5,
     xlab = "Threshold Value Index",
     ylab = "Number of Clusters",
     main = "Leading Eignevector Community Number of Clusters")
par(mfrow=c(2,2))
histP1(multilevel,
     breaks = 20,
     col = "darkgreen",
     xlab = "Number of Clusters",
     main = "Multilevel Community Cluster Distribution" )
histP1(walktrap,
     breaks = 20,
     col = "blue",
     xlab = "Number of Clusters",
     main = "Walktrap Community Cluster Distribution" )
histP1(fastgreed,
     breaks = 20,
     col = "red",
     xlab = "Number of Clusters",
     main = "Fastgreedy Community Cluster Distribution" )
histP1(leadingeig,
     breaks = 20,
     col = "black",
     xlab = "Number of Clusters",
     main = "Leading Eigenvector Community Cluster Distribution" )
```
...
V(g)$color <- rep("SkyBlue2",length(V(g)))
V(g)$size <- 0.02*(graph.strength(g))
E(g)$width = 0.02*E(g)$weight
...
# FILTER LABELS TO THE HIGHEST WEIGHTED DEGREE NODES
for (k in 1:vcount(g)){
  if (graph.strength(g)[k]<quantile(graph.strength(g),0.9)){
    V(g)$name[k] <- NA
  }
}
...
```{r, echo=FALSE,results='markup',warning=FALSE,message=FALSE,fig.width=12,fig.height=6,dpi=150,out.width='1200px',out.height='600px'}
g <- graph.adjacency(adjmatrix = edgem, 
                     mode = "undirected",
                     weighted = TRUE)
sh <- shortest.paths(g)
