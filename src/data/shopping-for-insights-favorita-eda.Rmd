...
This is an extensive Exploratory Data Analysis for the [Corporación Favorita Grocery Sales Forecasting](https://www.kaggle.com/c/favorita-grocery-sales-forecasting) competition with [tidy R](http://tidyverse.org/) and [ggplot2](http://ggplot2.tidyverse.org/).
...
The end of the last section already included the *city* and *type* of the store and found that these features have little impact on the *sales* boost from *promotions*. Now we will look at location properties in general.



We start with an overview *count plot* in which we plot *store\_nrb* vs *state* in a grid and add colour information for the store *type*. In addition, the size of the dots is (logarithmically) scaled to the total sales:
...
stores %>%
  ...
  left_join(foo, by = "store_nbr") %>%

  mutate(store_nbr = as.integer(store_nbr)) %>%

  ggplot(aes(store_nbr, state, colour = type, size = sum_sales)) +
...
- There is a certain association between *store\_nbr* and *type*; in particular between 10-20 for the "C" stores and between 40-50 for the "A" stores. "A" stores have large total sales, followed by "D" stores and "B" stores (compare Fig. 22).



- We recover the dominance of "Pichincha", which the first few store numbers (all "D" stores). The "Manabi" state contains all the highest *store\_nbrs*.
