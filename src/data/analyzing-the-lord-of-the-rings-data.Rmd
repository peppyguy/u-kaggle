...
Hi! In this kernel we are going to analyze two awesome data sets related to The Lord of the Rings: one containing the scripts from the movies, and the other one with information about the characters of the trilogy (gender, height, race, etc.). The analysis is going to be similar to the Star Wars kernel ([Analyzing Star Wars Movie Scripts](https://www.kaggle.com/xvivancos/analyzing-star-wars-movie-scripts)), but improved with new insights and visualizations. I've always liked the Lord of the Rings, so I'm keen to make this kernel. Let's start!
...
First we are going to use the general-purpose lexicon `bing`, from [Bing Liu and collaborators](https://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html). The `bing` lexicon categorizes words in a binary fashion into positive and negative categories. We can use the function `comparison.cloud()` to visualize the words. 
...
We can discover something curious with this visualization. The Two Towers is the movie with the most dialogues (as we saw in the previous
plot), but not the one that has more associated sentiments. Maybe it's because The Two Towers is a movie with more action. We can also use this lexicon to compute the most frequent words for each sentiment.
...
Finally, we are going to use the `AFINN` lexicon, from [Finn Årup Nielsen](http://www2.imm.dtu.dk/pubdb/views/publication_details.php?id=6010). This lexicon assigns words with a score that runs between -5 and 5, with negative scores indicating negative sentiment and positive scores indicating positive sentiment. Let's see the word distribution according to the `AFINN` lexicon.  
...
Let's generate some wordclouds to discover the most frequent words for each movie! Furthermore, in this section we are going to compute the most repeated bigrams and trigrams. What's a bigram? It's a sequence of two adjacent elements from a string of tokens, which are typically letters, syllables, or words. A trigram is the same but with three adjacent elements.
...
Many characters have this field empty, so we get less values. As we can see, Gondor is the kingdom with the most characters, followed by Númenor and Rohan. 
